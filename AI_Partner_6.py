...
def normalize_user_key(raw: str) -> str:
    """规范化输入，避免大小写/空格导致同一用户变成不同目录。"""
    raw = (raw or "").strip().lower()
    raw = re.sub(r"\s+", "", raw)
    return raw

def user_key_to_user_id(user_key: str) -> str:
    """
    将用户输入（邮箱/自定义ID）转为不可逆哈希，用作目录名，保护隐私。
    """
...
# 获取用户会话目录
def get_user_sessions_dir() -> str:
    """按用户输入生成独立会话目录 sessions/<user_hash>/"""
    user_key = st.session_state.get("user_key", "")
    user_id = user_key_to_user_id(user_key)
...
if "nick_name" not in st.session_state:
    st.session_state.nick_name = '请输入伴侣昵称'

#性格
if "nature" not in st.session_state:
    st.session_state.nature = '请输入伴侣性格'
...
with st.sidebar:
    ...
    user_key = st.text_input(
        "请输入邮箱或自定义ID（用于找回历史会话）",
        value=st.session_state.get("user_key", ""),
        placeholder="例如：yourname@email.com 或 oscarhe",
    )
    st.session_state.user_key = user_key

    # 没输入用户标识就不允许继续（防止所有人用同一默认目录）
    if not user_key_to_user_id(user_key):
        st.info("请输入邮箱/ID 后，才能加载/保存你的历史会话（不同用户互相不可见）。")
        st.stop()
